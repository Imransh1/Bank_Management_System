<?php
	...
	if($mob_no == $row['Mobile_no'] && $debitcard == $row['Debit_Card_No'] && $dbt_pin == $row['Debit_Card_Pin']){
		...
		// update otp into OLD password
		$otp_update = "Update bank_customers SET Password = '$otp' WHERE bank_customers.Customer_ID = '$customer_id '";

		$conn->query($otp_update);
		if($result=$conn->query($otp_update)== TRUE){
				echo '<script>alert("here is the new password generated please get this changes ASAP '.$otp.' \n\n Please verify")
